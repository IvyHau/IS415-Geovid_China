...
boxbreaks <- function(v, mult=1.5) { 
  qv <- quantile(v, probs=c(0.00, 0.25, 0.5, 0.75, 1.00))
  iqr <- qv[4] - qv[2]
...
boxmap <- function(vnam, df, legtitle=NA, mtitle="Box Map", mult=1.5){
  ...
  tm_layout(main.title = "Box Map of " + toString(vnam) + " at Municipality Level" , main.title.position = "center", main.title.size = 1.3, legend.position = c("right", "top"), legend.outside = TRUE , frame = FALSE) +
    tm_text("City_EN", size = 0.15)
}
```

## Boxmap of Confirmed Cases 
```{r echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE}
# confirmed_geo_sf[is.na(confirmed_geo_sf)] <- 0
# 
# var <- get.var("Covid'19 Rate (Per 10,000)", confirmed_geo_sf)
# qv <- unname(quantile(var))
# bb <- boxbreaks(var)
# 
# boxmap("Covid'19 Rate (Per 10,000)", confirmed_geo_sf)
```
